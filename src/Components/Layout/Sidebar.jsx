...
const Sidebar = () => {
    ...
    return (
      <div className="h-full border-n-6 border-solid border box-border bg-n-1 m-2 text-black rounded-md min-w-[20%] max-w-[25%] p-3 sm:p-0 sm:w-0 xl:w-[20%] lg:w-[20%] 2xl:max-w-[20%]">
        <div>
          ...
          <ul>
            {sidebarIcon.map((item, index) => (
              ...
              >
                <img
                  src={item.icon}
                  className={`w-6 h-6 mr-2 filter invert-0 transition-all duration-300 ${
                ...
                <span
                  className={`ml-1 transition-colors duration-300 ${
                    activeIndex === index ? 'text-n-1' : 'group-hover:text-n-1 text-n-3'
                  }`}
                >
                  {item.title}
                </span>
